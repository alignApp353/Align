...
const PERSONAL_DOMAINS=new Set(['gmail.com','googlemail.com','hotmail.com','outlook.com','live.com','icloud.com','me.com','mac.com','aol.com','yahoo.com','ymail.com','proton.me','protonmail.com','pm.me','gmx.com','mail.com','zoho.com']);
const ALLOWED_VOICES=new Set(['Professional','Friendly','Modern','Direct','Formal','Energetic']);
const COMPANY_PRIORITIES=['Career Growth','Competitive Pay','Company Culture','Learning & Development','Performance & Recognition','Meaningful Work','Innovation','Work-Life Balance'];
const CANDIDATE_PRIORITIES=['Technical Skills','Soft Skills','Experience','Communication','Culture Fit','Reliability','Leadership','Availability'];

function cleanText(value,max=5000){return String(value??'').trim().slice(0,max)}
function cleanDomain(url=''){return String(url).toLowerCase().replace(/^https?:\/\//,'').replace(/^www\./,'').split('/')[0].split(':')[0].trim()}
function sendJson(res,status,payload){res.status(status).json(payload)}
function getAccessToken(req){const value=req.headers.authorization||'';return value.startsWith('Bearer ')?value.slice(7).trim():null}
function uniqueStringArray(value,allowed){if(!Array.isArray(value))return[];return [...new Set(value.map(v=>cleanText(v,100)).filter(v=>allowed.has(v)))]}
function validateCompleteRanking(value,allowed,label){const cleaned=uniqueStringArray(value,new Set(allowed));if(cleaned.length!==allowed.length||allowed.some(v=>!cleaned.includes(v)))throw new Error(`${label} must include every available option once.`);return cleaned}
function randomAccountNumber(){const chars='ABCDEFGHJKLMNPQRSTUVWXYZ23456789';let value='ALY-';for(let i=0;i<6;i++)value+=chars[Math.floor(Math.random()*chars.length)];return value}
async function uniqueAccountNumber(){while(true){const account=randomAccountNumber();const{data}=await supabase.from('companies').select('id').eq('account_number',account).maybeSingle();if(!data)return account}}
function hashEin(ein){const secret=process.env.EIN_HASH_SECRET||process.env.SUPABASE_SERVICE_ROLE_KEY;return crypto.createHmac('sha256',secret).update(ein).digest('hex')}
...
function validatePayload(body){
  ...
  const ein=cleanText(body.ein,20).replace(/\D/g,'');if(!/^\d{9}$/.test(ein))throw new Error('Enter a valid 9-digit EIN.');payload.ein_last4=ein.slice(-4);payload.ein_hash=hashEin(ein);
  let websiteUrl;try{websiteUrl=new URL(payload.website.startsWith('http')?payload.website:`https://${payload.website}`)}catch{throw new Error('Please enter a valid company website.')}
...
async function insertBlueprint(companyId,payload){
  const modern={company_id:companyId,company_voice:payload.company_voice,company_priorities:payload.company_priorities,candidate_priorities:payload.candidate_priorities,ai_summary:payload.ai_summary||null};
  const modernResult=await supabase.from('company_blueprints').insert(modern);
  if(!modernResult.error)return;
  const schemaMissing=/column|schema cache|could not find/i.test(modernResult.error.message||'');
  if(!schemaMissing)throw new Error(`Unable to save the AI Hiring Blueprint: ${modernResult.error.message}`);
  const legacy={company_id:companyId,company_description:payload.ai_summary||'AI Hiring Blueprint created during employer onboarding.',company_values:payload.company_priorities.join(', '),work_environment:'Defined through ranked company priorities.',ideal_candidate:payload.candidate_priorities.join(', '),roles_hired:'Configured per job posting.',required_skills:payload.candidate_priorities.join(', '),poor_fit:'Evaluated against ranked hiring priorities.',writing_tone:payload.company_voice.join(', '),writing_emphasis:payload.company_priorities.join(', '),avoid_words:null,recommendation_style:'ranked',weight_skills:25,weight_culture:20,weight_experience:15,weight_availability:10,weight_location:10,weight_education:20};
  const legacyResult=await supabase.from('company_blueprints').insert(legacy);
  if(legacyResult.error)throw new Error(`Unable to save the AI Hiring Blueprint: ${legacyResult.error.message}`);
}

module.exports=async function handler(req,res){
  if(req.method!=='POST'){res.setHeader('Allow','POST');return sendJson(res,405,{success:false,error:'Method not allowed.'})}
  if(!process.env.SUPABASE_URL||!process.env.SUPABASE_SERVICE_ROLE_KEY)return sendJson(res,500,{success:false,error:'Server configuration is incomplete.'});
