...
async function manageCurrentUser(token){
  ...
  if(!r.ok||!data?.id){
    const e=new Error('Invalid employer session.');
    e.status=401;
    throw e;
  }
...
function exactPlan(ent){
  ...
  if(legacy==='enterprise'){
    return Number(ent?.slot_limit||0)>=8?'scale':'growth';
  }
...
function entitlementLooksActive(ent){
  ...
  const end=ent?.current_period_end
    ? new Date(ent.current_period_end).getTime()
    : Infinity;
...
function subscriptionPlan(ent,sub){
  ...
  return ['launch','growth','scale'].includes(fromStripe)
    ? fromStripe
    : exactPlan(ent);
}
...
function subscriptionBilling(ent,sub){
  ...
  return ['monthly','quarterly'].includes(fromStripe)
    ? fromStripe
    : billingPeriod(ent);
}
...
async function ensureRecurringPrice(plan,billing){
  ...
  const listed=await manageStripe(
    'GET',
    'prices',
    {
      'lookup_keys[]':p.lookup,
      active:'true',
      limit:1
    }
  );
  ...
  const created=await manageStripe(
    'POST',
    'prices',
    {
      currency:'usd',
      ...
      'recurring[interval_count]':p.intervalCount,
...
async function searchPlanSubscription(employerId){
  ...
  const found=await manageStripe(
    'GET',
...
async function resolveSubscription(employerId,ent){
  ...
  if(!sub){
    sub=await searchPlanSubscription(employerId);
  }
...
function subscriptionSummary(ent,sub){
  ...
  const p=terminal
    ? null
    : (MANAGE_CATALOG[billing]?.[plan]||null);
...
async function resolveSecondSlotSubscription(employerId){
  ...
  if(!sub){
    sub=await searchSubscriptionByProduct(employerId,'single_job');
  }
...
async function scheduleDowngrade({
  ...
  const intervalCount=
    MANAGE_CATALOG[billing][targetPlan].intervalCount;
  ...
  return {
    ...
    effective_at:new Date(end*1000).toISOString(),
...
async function applyPaidFixedUpgrade({
  ...
  await patchEntitlement(employerId,{
    ...
    subscription_status:
      String(updated.status||'active').toLowerCase(),
    ...
    // Alygnn Recommended belongs ONLY to Scale.
...
async function upgradeNow({
  ...
  const currentCatalog=
    MANAGE_CATALOG[billing]?.[currentPlan];

  const targetCatalog=
    MANAGE_CATALOG[billing]?.[targetPlan];

  if(!currentCatalog||!targetCatalog){
    throw new Error(
  ...
  const difference=Math.max(
    0,
    Number(targetCatalog.cents)-Number(currentCatalog.cents)
  );
  ...
  const targetPrice=
    await ensureRecurringPrice(targetPlan,billing);
  ...
  if(String(finalized?.status||'').toLowerCase()==='paid'){
    ...
    return {
      ...
      amount_due_cents:
        finalized.amount_due??difference,

      amount_paid_cents:
        finalized.amount_paid??difference,
  ...
  return {
    ...
    amount_paid_cents:
      finalized?.amount_paid??0,

    hosted_invoice_url:
      finalized?.hosted_invoice_url||null,

    payment_intent_status:
      finalized?.payment_intent?.status||null,
...
async function customerSnapshot(customerId,sub){
  ...
  try{
    ...
    invoices=(result?.data||[]).map(invoice=>({
      ...
      currency:
        String(invoice.currency||'usd').toLowerCase(),
...
function calculateNextPlanCharge(ent,sub){
  ...
  const pending=
    String(ent?.pending_plan||'').toLowerCase();
  ...
  return (
    MANAGE_CATALOG[billing]?.[currentPlan]?.cents??
...
async function runManageAction(res,user,input){
  ...
  if(action==='billing_portal'){
    ...
    const customerId=
      ...
      (
        typeof secondSlot?.customer==='string'
          ? secondSlot.customer
          : secondSlot?.customer?.id
      )||
      null;
  ...
  if(action==='cancel_scheduled_change'){
    ...
    return send(res,200,{
      ok:true,
      change:'scheduled_change_canceled'
    });
  }
  ...
  if(action==='cancel_second_slot'){
    const secondSlot=
      await resolveSecondSlotSubscription(user.id);
  ...
  const target=
    String(input.target_plan||'').toLowerCase();

  const current=
    subscriptionPlan(ent,sub);

  const currentBilling=
    subscriptionBilling(ent,sub);
  ...
  const targetBilling=
    ['monthly','quarterly'].includes(requestedBilling)
      ? requestedBilling
      : currentBilling;

  const currentCatalog=
    MANAGE_CATALOG[currentBilling];

  const targetCatalog=
    MANAGE_CATALOG[targetBilling];

  if(!targetCatalog?.[target]){
    return send(res,400,{
      error:'Choose Launch, Growth, or Scale with Monthly or Quarterly billing.'
    });
  }
  ...
  if(billingChanged){
    // Monthly <-> Quarterly changes take effect at renewal.
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(!product&&plan&&plan!=='single_job'){
      product='job_plan';
    }

    if(!product&&plan==='single_job'){
    ...
    if(product==='additional_slot'){
      ...
      days=Math.max(
        ...
        Math.min(
          ...
          Number.parseInt(
            input.days||
            Math.ceil(Number(input.duration_hours||24)/24),
            10
          )||1
        )
      );
      ...
      const item=
        CHECKOUT_CATALOG[billing]?.[plan];

      if(!item){
        return send(res,400,{
          error:'Unknown Alygnn plan or billing period.'
        });
      }
      ...
      if(billing==='monthly'||billing==='quarterly'){
        ...
        const currentBilling=
          String(access?.billing_period||'').toLowerCase();
    ...
    const metadata={
      ...
      source:
        String(input.source||'website'),

      terms_version:
        String(input.terms_version||'')
    };
    ...
    if(mode==='subscription'){
      ...
      params[
        ...
        product==='job_plan'&&billing==='quarterly'
          ? 3
          : 1;
    }
    ...
    const session=
      await stripeCreateCheckout(params);
